$(document).ready(function () {
  ...
  $(".categoria_item").click(function () {
    ...
    function mostrarProd() {
      ...
      $('.proyectos_item[category="' + categoriaProd + '"]').css(
        "transform",
        "scale(1)"
      );
    }
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  avanzarButton.addEventListener('click', function () {
      if (progreso < 100) {
          progreso += 10; // Puedes ajustar el incremento de progreso según tus necesidades.
          progresoElement.textContent = progreso + '%';
      }

      if (progreso === 100) {
          avanzarButton.disabled = true;
          avanzarButton.textContent = 'Lección Completada';
      }
  });
});
